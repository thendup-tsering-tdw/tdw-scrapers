...
baseUrl = 'http://maps.morinville.ca/Morinville/Views/SearchExport.aspx?type=Report&REPORTTYPE=221&FILEPATH=3210'
  ...
  (shardStart..shardEnd).each do |i|
    ...
    a.get(url) do |pdfFile|
      if pdfFile.body.match(/Failed to download file using the specified path/)
        STDERR.puts "File: 3210#{idx}2014_110.pdf not found"
      else
        ap "Saved: 3210#{idx}2014_110.pdf"
